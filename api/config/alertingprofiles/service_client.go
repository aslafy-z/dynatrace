...
func (cs *Service) Create(alertingProfile *AlertingProfile) (*api.EntityShortRepresentation, error) {
	var err error
	var bytes []byte

	if bytes, err = cs.client.POST(fmt.Sprintf("/alertingProfiles"), alertingProfile, 201); err != nil {
		return nil, err
	}
...
func (cs *Service) Update(alertingProfile *AlertingProfile) error {
...
func (cs *Service) Get(id string) (*AlertingProfile, error) {
	var err error
	var bytes []byte
	...
	var alertingProfile AlertingProfile
