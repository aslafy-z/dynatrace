...
func (cs *ServiceClient) Create(mw *MaintenanceWindow) (*api.EntityShortRepresentation, error) {
	var err error
	var bytes []byte
...
func (cs *ServiceClient) Update(mw *MaintenanceWindow) error {
	if len(opt.String(mw.ID)) == 0 {
...
func (cs *ServiceClient) Get(id string) (*MaintenanceWindow, error) {
	if len(id) == 0 {
